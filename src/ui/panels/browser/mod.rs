            ...
            |cx| {
                ...
                Panel::new(
                    ...
                    |cx| {
                        ...
                        Button::new(cx, |cx| cx.emit(BrowserEvent::SelectNext), |cx|{
                            Label::new(cx, "Next")
                        });
                        Button::new(cx, |cx| cx.emit(BrowserEvent::SelectPrev), |cx|{
                            Label::new(cx, "Prev")
                        });
                    },
                    |cx| {
                        ...
                        VStack::new(cx, |cx| {
                            directory(cx, UiData::state.then(UiState::browser.then(BrowserState::root_file)), 0, directory_header, |cx, item, level|{
                                directory(cx, item, level, directory_header, |cx, item, level|{
                                    directory(cx, item, level, directory_header, |cx, item, level|{
                                        directory(cx, item, level, directory_header, |cx, item, level|{
                                            directory(cx, item, level, directory_header, |cx, item, level|{
                                                let file_path1 = item.clone().get(cx).file_path.clone();
                                                let file_path2 = item.clone().get(cx).file_path.clone();
                                                let file_path3 = item.clone().get(cx).file_path.clone();
...
where
    L: Lens<Target = File>,
{
    let file_path1 = root.get(cx).file_path.clone();
    let file_path2 = root.get(cx).file_path.clone();
    let file_path3 = root.get(cx).file_path.clone();
    HStack::new(cx, |cx| {
        ...
        Label::new(cx, "\u{e75c}")
            ...
            .rotate(root.clone().then(File::is_open).map(|flag| if *flag { 0.0 } else { -90.0 }));
        // File or directory name
        Label::new(cx, root.clone().then(File::name)).width(Stretch(1.0)).text_wrap(false);
    })
    .class("dir-file")
    .toggle_class(
        "focused",
    ...
    L::Source: Model,
{
    ...
    VStack::new(cx, |cx| {
        ...
        let root = lens.clone();
        VStack::new(cx, |cx| {
            List::new(cx, lens.clone().then(File::children), move |cx, index, item| {
                (content.clone())(cx, item.clone(), level + 1);
            })
            .height(Auto);

            let file_path1 = lens.clone().get(cx).file_path.clone();
            Element::new(cx)
